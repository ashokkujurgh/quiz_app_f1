...
3. Fetch trending Wikipedia topics and match to subtopics
4. Generate title + content (trending-aware)
5. Embed title+content → check Pinecone for duplicates
6. Generate DALL-E image
...
from trending import fetch_wikipedia_trending, match_trending_to_subtopic
...
def run_agent() -> None:
    ...
    # 3. Fetch trending titles once (shared across all subtopics this run)
    ...
    for subtopic in selected:
        ...
        # 4. Find trending match for this subtopic
        trending_hint = match_trending_to_subtopic(subtopic, trending_titles)
        if trending_hint:
            logger.info("Trending hint applied: %s", trending_hint)

        # 5. Generate title + content
        generated = generate_title_and_content(topic_name, subtopic_name, trending_hint)
        if not generated:
        ...
        if exists:
            logger.info(
                "Duplicate post detected (matched=%s) for '%s'. Skipping.",
